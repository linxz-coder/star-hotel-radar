...
- Trip.com 搜索默认渐进式返回：先返回任务进度；有缓存先秒出缓存，有候选先展示候选，后台通过 SSE 推送继续刷新实时价格和完整比价
- 搜索结果会明确标记“已找到但未完成”，并分层展示整页搜索缓存、候选元数据缓存、酒店日期价格缓存的命中情况
...
酒店日期价格另有独立缓存：已确认是含税总价的 `hotelId + 入住日期` 会写入 MySQL 的 `hotel_price_cache` 表，默认 24 小时有效。写入范围包含用户选择的入住日期，也包含 4 个对比日期的价格；后续搜索同一家酒店同一天价格时，会先使用这张表，避免重复打开列表页或详情页，也避免每次重新跑完整对比日期抓取。如果用户勾选“重新实时搜索”，本次搜索会跳过价格缓存读取并重新抓取 Trip.com，但新抓到的含税价仍会写回缓存，用来修正旧价格。
...
为了让用户更早看到结果，前端默认使用渐进式异步搜索：`/api/search` 会立即返回 `summary.jobId` 和 `summary.progress`；前端随后订阅 SSE 结果流。如果缓存命中，会先展示缓存并用 `summary.refreshJobId` 后台刷新；如果冷抓中间拿到候选酒店，会先展示目标日期候选和当前价；后台完整比价完成后自动替换为最终结果。需要同步调试时可传 `asyncMode=0`。

当前默认使用 SSE：前端订阅 `/api/search/events/<jobId>`，后台页订阅 `/api/admin/events`，任务状态变化时服务端主动推送。浏览器不支持 SSE 或连接失败时才退回低频状态接口轮询。
